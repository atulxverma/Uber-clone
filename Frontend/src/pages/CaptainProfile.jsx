  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-white p-6 pb-4 flex items-center justify-between shadow-sm sticky top-0 z-10">
        <button
          onClick={() => navigate(-1)}
          className="h-10 w-10 bg-gray-100 flex items-center justify-center rounded-full hover:bg-gray-200 transition-all"
        >
      ...
      <div className="p-5 flex-1 overflow-y-auto pb-10">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 flex flex-col items-center mb-6 relative">
          {!isEditing && (
            <button
              ...
              className="absolute top-4 right-4 text-blue-600 bg-blue-50 p-2 rounded-full font-medium text-sm flex items-center gap-1 active:scale-95"
            >
          ...
          <div className="relative mb-4">
            <div className="h-24 w-24 bg-yellow-400 rounded-full flex items-center justify-center text-4xl font-bold text-gray-800 shadow-lg overflow-hidden border-4 border-white">
              {profilePic || captain?.profilePic ? (
                <img
                  src={profilePic || captain?.profilePic}
                  alt="profile"
            ...
            {isEditing && (
              <button
                ...
                className="absolute bottom-0 right-0 bg-blue-600 text-white h-8 w-8 rounded-full flex items-center justify-center shadow-md border-2 border-white"
              >
          ...
          {isEditing ? (
            <div className="w-full mt-2 flex flex-col gap-3">
              <input
                ...
                className="bg-gray-100 p-3 rounded-xl outline-none focus:border-black"
              ...
              <input
                ...
                className="bg-gray-100 p-3 rounded-xl outline-none focus:border-black uppercase"
              ...
              <div className="flex gap-2 mt-2">
                <button
                  onClick={() => {
                    setIsEditing(false);
                    setProfilePic(captain?.profilePic || "");
                  }}
                  className="w-1/2 bg-gray-200 text-gray-700 py-3 rounded-xl font-bold"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={isLoading}
                  className="w-1/2 bg-green-600 text-white py-3 rounded-xl font-bold"
                >
            ...
            <>
              ...
              <div className="flex items-center justify-center gap-3 mt-2 text-sm text-gray-600 font-medium">
                <span className="bg-gray-100 px-3 py-1 rounded-md uppercase">
                  {captain?.vehicle?.plate}
                </span>
                <span>•</span>
              ...
              <div className="mt-4 bg-green-100 text-green-700 px-4 py-1.5 rounded-full text-sm font-bold flex items-center gap-1">
                <i className="ri-shield-check-fill text-lg"></i> Authorized
                Partner
              </div>
            </>
          )}
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 flex items-center justify-between mb-6">
          <div>
            ...
            <p
              className={`text-sm font-medium ${isOnline ? "text-green-600" : "text-red-500"}`}
            >
          ...
          >
            <div
              className={`absolute top-1 left-1 bg-white w-6 h-6 rounded-full shadow-md transform transition-transform duration-300 ${isOnline ? "translate-x-8" : "translate-x-0"}`}
            ></div>
          </button>
        </div>

        <h3 className="text-gray-500 font-bold text-xs mb-3 uppercase tracking-wider px-2">
          Work Dashboard
        </h3>

        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden mb-6">
          <div
            className="flex items-center justify-between p-4 border-b border-gray-100 active:bg-gray-50 transition-all cursor-pointer"
        ...
        <Link
          to="/captain/logout"
          className="w-full bg-red-50 text-red-600 font-bold py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-red-100 transition-all shadow-sm"
        >
